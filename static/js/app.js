...
function markVideoAspect(v) {
  ...
  v.dataset.aspect = ratio < 1 ? "9:16" : (ratio > 1.3 ? "16:9" : "1:1");
  if (v.videoWidth < v.videoHeight) v.classList.add("portrait");
  ...
  app.innerHTML = `<div class="wrap">
    <div class="card"><h2>工作台</h2><div class="sub">选择一种方式开始制作你的数字人短视频</div>
      <div class="entry-grid">
        ...
        <div class="entry" id="eLib"><div><div class="ico">📚</div><h3>我的文案库</h3><p>查看所有提取/保存的文案（含智能分类与真实视频数据），点开可复用改写。</p></div><div class="muted">入口三 →</div></div>
      </div>
    </div>
  </div>`;
  bindLogout();
...
async function pageExtract(app) {
  ...
  app.innerHTML = `<div class="wrap">
    <div class="card"><h2>链接提取文案</h2><div class="sub">粘贴爆款视频链接，自动提取口播文案（百炼 Paraformer 转写）+ 智能分类</div>
...
async function pageRewrite(app) {
  ...
  app.innerHTML = `<div class="wrap cols-rewrite">
    ...
    ${hasGen ? `<div class="card" style="margin-top:16px"><h2 style="font-size:16px">✅ 改写稿<span class="muted" style="font-size:12px;font-weight:400;margin-left:8px">可继续编辑，配音默认用这份${script.updated_at ? " · 上次更新 " + String(script.updated_at).slice(5, 16) : ""}</span></h2>
...
async function pageDubbing(app) {
  ...
  app.innerHTML = `<div class="wrap">
    <div class="card"><h2>配音</h2><div class="sub">上传你的音色，调节情绪 / 语速 / 音调 / 音量，生成专属配音</div>
      ...
      <label style="margin-top:16px">随机种子（同一 seed 可复现；换一个就是换一版效果，不满意点🎲再抽一次）</label>
...
async function pageDigital(app) {
  ...
  app.innerHTML = `<div class="wrap cols-rewrite">
    ...
    <div class="card" style="margin-top:0">
      <h2 style="font-size:16px">口播视频预览</h2>
      <div id="dhPreviewCurrent"><div class="muted" style="padding:24px 0;text-align:center">点击左侧生成口播视频，结果会出现在这里</div></div>
      <div id="dhHistoryHead" style="display:none;margin-top:16px;padding-top:14px;border-top:1px solid var(--line2)">
        <div style="font-size:13px;font-weight:700;color:var(--text)">历史生成记录</div>
      </div>
      <div id="dhPreviewHistory"></div>
    </div>
  </div>`;
  bindLogout();
  ...
  function dhMediaHTML(r, cls="media") {
    if (r.video_url) return `<video class="${cls} auto-pip" controls src="${r.video_url}"></video>`;
  ...
  function renderDHHistory(list) {
    const box = document.getElementById("dhPreviewHistory");
    const head = document.getElementById("dhHistoryHead");
    if (!list || !list.length) {
      ...
      <div class="dh-history-item">
        ...
        <div class="dh-h-info">
          ...
          <div class="row" style="margin-top:6px">
            <button class="btn ghost sm" onclick="go('#/editing?vid=${r.video_id}')">去剪辑</button>
          </div>
        </div>
      </div>
    `).join("");
...
async function pageEditing(app) {
  const { vid } = qp();
  STATE.video_id = vid;
  // 剪辑效果开关。定版（老板 2026-09-02）：字幕默认常驻普通字幕（即此前调试效果）；
  // "网感大字"卡默认不勾选 = 普通字幕，勾选 = 未来爆款大字（后台暂未实现，仍走普通字幕）。
  const opts = { color: true, bigtext: false, mg: true, bgm: true };
  const fxOpts = [
